...
select sum(salary) 합계, round(avg(salary),2) 평균, max(salary) 최대값, min(salary) 최소값
from employee
where job !='SALSMAN' and job !='PRESIDENT' and job !='CLERK';
...
select sum(salary) "부서별 월급 합계", round(avg(salary),2) "부서별 월급 평균",
...
select ename 사원명,job 직책,salary 월급, dno 부서번호
from employee
...
select eno 사원번호, ename 사원명, job 직책, salary 월급
from employee
